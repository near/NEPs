...
NEAR and third-party applications need to track `mint`, `update`, `transfer`, `burn`, and `contract_metadata_update` events for all NFT-driven apps consistently.
...
Non-Fungible Token Events MUST have `standard` set to `"nep171"`, standard version set to `"1.2.0"`, `event` value is one of `nft_mint`, `nft_update`, `nft_burn`, `nft_transfer`, or `contract_metadata_update`, and `data` must be of one of the following relevant types: `NftMintLog[] | NftUpdateLog[] | NftTransferLog[] | NftBurnLog[] | NftContractMetadataUpdateLog[]`:

```ts
interface NftEventLogData {
    standard: "nep171",
    version: "1.2.0",
    event: "nft_mint" | "nft_update" | "nft_burn" | "nft_transfer" | "contract_metadata_update",
    data: NftMintLog[] | NftUpdateLog[] | NftTransferLog[] | NftBurnLog[] | NftContractMetadataUpdateLog[],
}
```
...
// An event log to capture token updating (e.g. new metadata)
// Arguments
// * `token_ids`: ["1", "abc"]
// * `memo`: optional message
interface NftUpdateLog {
    token_ids: string[],
    memo?: string
}
...
1. Events that do not have a dedicated trigger function in the NFT Standard (`nft_mint`, `nft_update`, `nft_burn`, `contract_metadata_update`)
