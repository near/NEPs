...
it's now much cheaper to reuse the same code across different contracts in terms
of storage costs. It opens up a door for sharded contracts design where each
instance needs to be deployed on a separate account.
...
If a contract needs to authenticate predecessor ID and verify that it
was executing the [same code](#other-host-functions), then it can simply
...
let state_init = StateInit {
    ...
    data: Self::init_state_for(caller_params),
};
...
Moreover, such verification logic can be applied to all deterministic accounts,
not just the predecessor, enabling for infinite composability between different
deterministic and non-deterministic contracts.
...
of Sharded Fungible Token contracts with deterministic account ids along with
definition of [`StateInit`](#deterministic-accountids) structs and required [host functions](#stateinit-action).  
All changes are **backwards-compatible** with existing contracts and account
ids.
...
in TON blockchain.  
However, we still needed to adapt their definitions to Near specifications:
...
e.g. 2FA, social recovery, gasless transactions and many [more](https://github.com/ton-blockchain/wallet-contract-v5/blob/main/README.md#suggested-extensions).  
...
Deterministic accounts are allowed to freely interact with non-deterministic
ones and vice versa.  

Existing contracts can be upgraded to start using [`StateInit` action](#stateinit-action)
and, thus, gain the power to automatically deploy non-existing deterministic
accounts.
