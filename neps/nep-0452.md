...
A user with _no_ wallet that is claiming the assets associated with an existing public key, already registered on the linkdrop contract:
...
A user with an _existing_ wallet that is claiming the assets with an existing public key, already registered on the linkdrop contract:
