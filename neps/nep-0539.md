...
Congestion is primarily tracked in terms of the gas of all delayed receipts.
...
cross-contract calls and delegated receipts, is limited by the receivers overall
...
Using either the previous chunk header or the state, and the list of receipts 
...
Transaction selection is changed to reject new transactions when the system is
...
But more importantly, we add a more targeted rule to reject all transactions to
...
much memory to store unprocessed receipts, or there is already too much work
...
2. When user demand is consistently higher than the system can handle, this
   mechanism lets backpressure propagate shard-by-shard until the shards
   responsible for accepting too many receipts are reached and transaction
   filtering kicks in.

To accomplish this, we add 3 new steps to chunk execution (enumerate as 1, 2, 6
...
1. (new) Read congestion information for the own shard and for all other shards
   from the previous chunk headers.
