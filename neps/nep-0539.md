...
We propose to limit the transactions and outgoing receipts included in each chunk. Limits depend
...
But these rules do not put any limits on what other shards would accept.
For example, when a particular contract is popular, the contract's shard will
eventually stop accepting new transactions, but all other shards will keep accepting
...
The proposal adds fields to chunks headers, adds a new trie column, changes
the transaction selection rules, and changes the chunk execution flow. The
next four sections specify each of these changes in more detail.
...
chunk header, increasing it from 374 bytes up to 422 bytes in the borsh representation
(assuming no validator proposals are included.) This in turn increases the block size
by up to 48 bytes per shard, as all chunk headers are fully included in blocks.

Including all this information in the chunk header enables efficient validation.
Using the previous chunk header (or alternatively, the state), combined with the list of receipts
applied and forwarded, a validator can check that the congestion rules described 
in this NEP are fulfilled.
...
`incoming_congestion` variable (between 0 and 1) computed in the previous chunk of the same shard:
...
def filter_tx(tx):
  ...
  if congestion(tx.receiver_shard_id) > REJECT_TX_CONGESTION_THRESHOLD
    tx.reject()
  else
    tx.accept()
```

Here, `congestion()` is the maximum of incoming, outgoing, and memory congestion.
...
1. It spreads the memory consumption from the congested shard to the shards
   producing the excess workload.
...
1. (new) Read congestion information for *all* shards from the previous chunk headers.
   ...
   ```rust
   // congestion info for each shard
   {
...
2. (new) Compute bandwidth limits to other shards based on the congestion information.
   The formula is:
    ...
    for receiver in other_shards:
      ...
      outgoing_congestion = outgoing_gas[receiver] / MAX_CONGESTION_OUTGOING_GAS

      MAX_CONGESTION_MEMORY_CONSUMPTION = 1000 MB
      memory_congestion = receipt_bytes[receiver] / MAX_CONGESTION_MEMORY_CONSUMPTION

      congestion = max(incoming_congestion, outgoing_congestion, memory_congestion)
...
itself is full congested. This gives fairness properties under certain workloads
...
Option 2 is preferred by the authors because it adds less overhead to the
state. It's important in light of stateless validation where we aim to keep 
the state witness as small as possible. 

#### Option 1 - store the total size and total gas in the state

In this option we store the total gas and total size of the incoming queue and 
the outgoing receipts in the state along the respective queue or buffers. Those
values will be updated as receipts are added or removed from the queue. 

In this case the CongestionInfo struct can remain small and only reflect the 
information needed by other shards. 
...
#### Option 2 - store the total size and total gas in the chunk extra
...
However, if the available chunk of a remote shard is more than 3 block heights
behind, we replace the congestion information with the maximum-congestion
values. This is to prevent sending even more receipts to a shard that already
struggles to produce chunks.

```rust
// max congestion for > 3 missing chunks
delayed_receipts_gas = u128::MAX;
buffered_receipts_gas: u128::MAX;
receipt_bytes: u64::MAX;
// nobody can send
allowed_shard: shard_of_chunk;
```
...
The preferred Option 2 in `Efficiently computing the congestion information` 
creates a dependence on the previous block when processing a block. For a 
fully synced node this requirement is always fulfilled because we keep at 
least 3 epochs of blocks. However in state sync we start processing from an
arbitrary place in chain without access to full history. 
