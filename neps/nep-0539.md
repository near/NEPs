...
a shard with an `memory_congestion` above a certain threshold.

```python
def filter_tx(tx):
  REJECT_TX_CONGESTION_THRESHOLD = 0.25
  if memory_congestion(tx.receiver_shard_id) > REJECT_TX_CONGESTION_THRESHOLDD
    tx.reject()
  else
    tx.accept()
```
...
TODO(wacban): do we need to describe how it works with resharding?
