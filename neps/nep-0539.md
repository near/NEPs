...
ShardChunkHeaderInnerV3 {
  ...
  receipt_bytes: u64,
  /// If fully congested, only this shard can forward receipts to us
  /// in the next chunk.
  allowed_shard: u64,
}
```

This adds 48 bytes to the chunk header, increasing it from 374 bytes to 422
bytes in borsh representation. (Assuming no validator proposals included.) This
in turn increases the block size by 48 bytes per shard.

Include all this information in the chunk header enables efficient validation.
Using only the previous chunk header and the list of receipts applied and
forwarded, a validator can check that the congestion rules st up in this NEP are
fulfilled.
...
5. Execute receipts in the order of `local`, `delayed`, `incoming`, `yield-resume time-out`.
    ...
    - (new) For all postponed or yielded receipts that are executed, remove
      `receipt.size()` from `receipt_bytes`.
...
- `receipt_bytes`: must be equal to `receipt_bytes` of the previous chunk, plus
  all bytes of new receipts added to delayed, postponed or yielded receipts, minus all the receipts of the same type removed.
