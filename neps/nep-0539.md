...
ShardChunkHeaderInnerV3 {
  ...
  /// were delayed, buffered, or yielded.
  receipt_bytes: u64,
...
5. Execute receipts in the order of `local`, `delayed`, `incoming`, `yield-resume time-out`.
    ...
    - (new) For all yielded receipts that are executed, remove `receipt.size()`
      from `receipt_bytes`.
...
- `receipt_bytes`: must be equal to `receipt_bytes` of the previous chunk, plus
  all bytes of new receipts added to delayed, or yielded receipts, minus all the
  receipts of the same type removed.
