...
TODO: Link to an actual implementation in nearcore will follow and be linked here.
...
pub struct StoredReceiptsInfo {
    /// Action execution gas of all receipts in the delayed receipts queue. This
    /// amount is guaranteed to be burnt when working through the queue.
    pub delayed_receipts_exec_gas: Gas,
    /// Gas attached to function calls in the delayed receipts queue.
    pub delayed_receipts_attached_gas: Gas,
...
```rust
let gas = delayed_receipts_exec_gas + delayed_receipts_attached_gas;
