...
Adding a second `Transfer` action allows the combination of nonrefundable
balance and refundable balance. This allows the user to make calls where they
need to attach balance, for example an FT transfer which requires 1 yocto NEAR.

```json
"Actions": {
  "CreateAccount": {},
  "Transfer": { "deposit": "1000000000000000000000000", "nonrefundable": true },
  "Transfer": { "deposit": "100", "nonrefundable": false },
  "AddKey": { "public_key": "...", "access_key": "..." }
}
```

To create implicit accounts, the current protocol requires a single `Transfer`
action without further actions in the same transaction. With this proposal,
setting it to non-refundable will make the full balance non-refundable. If users
want to add refundable balance on top, it requires a second transaction.
