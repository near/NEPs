# NEP-486 Precompile for BLS12-381 curve operations

---
NEP: 486
Title: Precompile for BLS12-381 curve operations
Author: Olga Kuniavskaia [olga.kunyavskaya@aurora.dev](mailto:olga.kunyavskaya@aurora.dev)
DiscussionsTo: [https://github.com/nearprotocol/neps/pull/486](https://github.com/nearprotocol/neps/pull/486)
Status: Draft
Type: Runtime Spec
Category: Contract
Created: 17-Jul-2023
...
The BLS12-381[1, 11, 52] is a wildly used[2-6, 7] elliptic curve with 120+ bits of security[8] which support **the *pairing* operation*.* It is a good alternative for bn254 elliptic curve[9, 12], which also supports the aggregation, and is currently implemented as NEAR precompiles[10]. Recent research shows that it contains only <100 bits of security[13] and we can see the tendency of switching from bn254 to bls12-381(ZCash[14], Ethereum[15], Tezos[16]).
...
**Definition:** The field $F_p$ for some *prime* $p$ is a set of integer elements $\{0, 1, \ldots, p - 1\}$ with two operations: multiplication $\cdot$ and addition $+$. These operations are performed as multiplication/addition for integers number and then taking the remainder modulo $p$.
...
For our BLS12-381 Elliptic Curve, **the order r** of $G1$  and $G2$([1, 2]):
...
In most cases we will work with points from $G'_2 \subset E'(F_{p^2})$ and use for this subgroup just the notation $G_2$.

### Generators

**Definition:** if in the group $G$ exists element g, such as $\{g, 2g, 3g, \ldots, |G|g\} = G$, the group G called ***cyclic group*** and g called ***generator***
...
***Output:*** the output is `96 bytes` — one point $\in G_1 \sub E(F_p)$ in decompressed format. More details are in the Curve Points Encoding section.
...
***Output:*** the output is `192 bytes` — one point $\in G_2 \sub E'(F_{p^2})$ in decompressed format. More details are in the Curve Points Encoding section.
...
The pairing function $e\colon G_1 \times G_2 \rightarrow G_T$, where $G_T \sub F_{q^{12}}$. The pairing function has the following properties:
...
This function takes as input the sequence of pairs $(p_i, q_i)$, where $p_i \in G_1 \sub E(F_{p})$ and $q_i \in G_2 \sub E'(F_{p^2})$ and check:
...
***Input:*** the sequence of pairs $(p_i, q_i)$, where $p_i \in G_1 \sub E(F_{p})$ and $q_i \in G_2 \sub E'(F_{p^2})$. Each point is encoded in decompressed form. Expected `288*k` bytes as an input that is interpreted as byte concatenation of `k` slices, each slice is the concatenation of uncompressed point from $G_1 \sub E(F_p)$ — `96 bytes` and point from $G_2 \sub E'(F_{p^2})$ — `192 bytes`. More details are in the Curve Points Encoding section.
...
The previous NEP for supporting BLS signature based on BLS12-381[26]: [https://github.com/nearprotocol/neps/pull/446](https://github.com/nearprotocol/neps/pull/446)
