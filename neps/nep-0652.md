...
This proposal improves shard liveness by reassigning future chunk slots away from a persistently
failing producer during the current epoch. The producer is excluded only from chunk production on
the affected shard; it is not immediately removed from the validator set. The existing
end-of-epoch kickout rules still decide whether it loses its validator seat two epoch boundaries
later. The implementation names the feature `ProtocolFeature::EarlyKickout`; this document uses
*reassignment* for the in-epoch mechanism and reserves *kickout* for the existing end-of-epoch
rules.
...
i.e. the chunk's *grandparent*. The assignment is sampled at `A.height + 2` and stored under `A`.
This remains true when skipped heights make `H` greater than `A.height + 2`: the row is
materialized when `A` is processed, before the eventual chunk height is known, and later readers
use that row verbatim instead of resampling at `H`. The grandparent, rather than the parent, is the
anchor because chunks are produced and distributed concurrently with the propagation of their
parent block: a node must be able to resolve a chunk's producer — to verify its signature —
before it has fully processed `P`, and the grandparent is the newest block it is guaranteed to
have processed by then.
...
* The grace check MUST derive the epoch start by walking backward from the basis along that fork's
  block-info chain. It MUST NOT use a shared per-epoch `EpochStart` record: boundary blocks on
  sibling forks can write the same epoch-id key with different first-block heights. Whichever
  sibling a node processes last would determine the shared value, so two nodes could make
  different grace-window decisions for the same anchor. Walking the basis's ancestry makes the
  epoch start fork-specific and deterministic.
...
Let `T` be the epoch in which the producer is excluded:

* At the start of `T+1`, the blacklist from `T` disappears because chunk statistics reset. The
  producer receives chunks again if the `T+1` settlement — fixed when `T-1` was finalized —
  still assigns it to the shard.
* If the producer is still failing, it can be excluded again during `T+1`, but the grace window,
  both exclusion thresholds, and the keep-one valve all apply afresh. An epoch too short to
  accumulate 100 misses cannot exclude it.
* Separately, `T`'s final statistics feed the existing end-of-epoch kickout. Subject to the
  exemptions and validator-selection conditions above, failing that check removes the producer
  from the validator set at the start of `T+2`.

An alternative that decouples assignment from these statistics is discussed under Alternatives.
A probe that lets a recovered producer return during the epoch is discussed under Future
possibilities.
...
required; see Detection latency for what that means in time. In the reference implementation all
three values are compile-time constants of the epoch manager — not epoch-config fields,
operator-tunable values, or per-chain settings. One consequence is that chains whose epoch length
...
* **Epoch sync.** A node joining through epoch sync installs the first block of the current epoch
  and block information for a few blocks of the previous epoch, including its first and final two
  blocks. The current epoch's first-block rows are seeded with an empty blacklist, which is correct
  because that epoch has no statistics yet.

  The previous epoch's complete aggregator cannot be reconstructed from this partial history.
  The epoch manager therefore starts an intentionally incomplete previous-epoch aggregator at the
  sync point. Accounting over the remaining tail may not reproduce every reassigned producer, but
  this does not affect validator selection: the epoch-sync proof already supplies the epoch
  information produced from the complete previous-epoch statistics, including
  `next_next_epoch_info`, and aggregation starts fresh at the current epoch's first block.
* **Statistics aggregation.** The `(produced, expected)` counters feed the end-of-epoch kickout that
  builds the future validator set, so ordinary block processing must attribute them to the producer
  consensus selected. Aggregation normally reads the assignment stored under the chunk's anchor.

  * If the anchor exists but its row is absent, or the stored account is not in the epoch,
    aggregation falls back to canonical sampling at `A.height + 2`. A healthy same-epoch database
    should never take this fallback: the row is written and garbage collected with the anchor's
    block info. A dedicated test verifies that every anchor with a non-empty blacklist has its rows.
  * If the parent or anchor block info is missing, the parent is genesis, or the anchor belongs to
    another epoch, the anchored helper returns no assignment and aggregation samples canonically at
    the parent's height plus one. This path is not limited to epoch sync: the first two blocks after
    every epoch boundary use it because their anchor is absent or belongs to the previous epoch.
    Canonical sampling is correct there because the consensus reader resolves those chunks
    canonically too.

  These aggregation fallbacks do not weaken consensus resolution. A missing same-epoch row on the
  consensus path still fails closed with `EpochError::ChunkProducerNotInDB`.
...
* **Parameter values.** The 100-miss floor, the 80% ratio, and the 1000-block grace window were
  ...
  and noise rejection. No historical replay of mainnet downtime was performed. Once the mechanism
  is live, the observability described above (reassignment rates, safety-valve firings) gives
  direct evidence for revisiting the values.
