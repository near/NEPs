...
* **Statistics aggregation.** The aggregation of `(produced, expected)` itself resolves producers
  ...
  the boundary kickout reads, so they are safe only while the governing invariant holds: the set of
  anchors that can be missing a row and the set of anchors with a non-empty blacklist are disjoint.
  Before activation, at genesis, for a cross-epoch anchor and at an epoch-sync boundary the
  blacklist is empty by construction, and a dedicated test pins the invariant.
