...
freezing the producer's chunk statistics, which are the input the end-of-epoch kickout reads when
it builds the validator set two epochs out.
...
also fails the boundary check, and mid-epoch exclusion settles how that check reads it. Two
existing exemptions still stand between failing the check and losing the seat: a validator inside
the stake exempted by `validator_max_kickout_stake_perc` is skipped before its statistics are
examined at all, and if every validator would otherwise be kicked out, one is kept. A producer
that neither exemption covers loses its seat, and with it the epoch's rewards, since kicked-out
validators are not rewarded. The proposal accepts this: a producer that has missed at least 100
...
anyone. Meanwhile `T`'s frozen statistics fail the boundary check, and, subject to the exemptions
above, the producer loses its seat from the start of `T+2`. An alternative that decouples assignment
...
* **Statistics aggregation.** The aggregation of `(produced, expected)` itself resolves producers
  ...
  fallback occurs only in states where canonical and anchored resolution are provably
...
still fails a boundary threshold at 80% or above. Failing that check selects it for kickout two
boundaries later, under the same exemptions that apply to any other failing producer (see
Interaction with end-of-epoch kickout and rewards).
...
normal parent-known path before it can affect block or chunk validation. Four constraints hold
there. Every message must carry a valid chunk producer signature for the referenced epoch and
shard, verified against the anchored producer. The chunk height must be exactly two above the
claimed anchor's height: a lower height is rejected outright, and a higher one is deferred until
the parent block is known rather than resolved through the anchor. An anchor the node cannot
resolve is not guessed at — the message is dropped. And the claimed epoch is checked for
plausibility against the node's own head, through the same `possible_epochs_of_height_around_tip`
list the pre-existing relevance check uses, so a message claiming an epoch that cannot contain its
height is dropped before resolution. A stricter rule, dropping a message outright whenever the
claimed anchor could sit at an epoch tail — where the claimed epoch would not determine the
producer — is proposed in nearcore [#16272](https://github.com/near/nearcore/pull/16272).
...
end-of-epoch kickout and reward outcome. Both outcomes are computed while the failing epoch is
finalized, and both land two boundaries later: that is when the new validator set takes over and
when the epoch's reward, carried in that set's epoch information, is minted.
...
* No changes to chunk headers, block headers, transactions, or any contract-visible or RPC schema.
  ...
  schema — of RPC endpoints that report producer assignments reflects this proposal. Endpoints that
  report an already-processed chunk's author return the anchored value, matching what consensus
  used. Forward-looking endpoints, such as the validator maintenance windows and the transaction
  routing target, still predict future assignments through the canonical sampler, so their answers
  can diverge from the eventual anchored producer while a blacklist is active on that shard.
...
* Exclusion is sticky for the remainder of the epoch regardless of the underlying cause, and by
  freezing the chunk statistics it settles how the end-of-epoch check reads them: a producer whose
  only assignment is the affected shard fails that check and, subject to the kickout exemptions,
  loses its seat two epochs out. This is intentional; see Interaction with end-of-epoch kickout
  and rewards.
...
1. **Database version bump.** Introducing `DBCol::ChunkProducers` comes with a `DB_VERSION` bump
   and its migration. An existing database is below the new version, and the version check runs
   before any read, so the first open must be read-write and must run the migration; that migration
   registers the new column family, and every later open — including a read-only one — then finds
   it. The migration creates the column empty; no historical data is backfilled, because rows are
   only meaningful for blocks processed under the new protocol version, and pre-activation chunks
   resolve through the canonical sampler.
...
fails closed on its own checks. Chunk parts have one refinement: when the parent block is neither
processed nor the subject of an outstanding request, the receiver falls back to an epoch guessed
from its own head, and a failure against that guess is reported as a not-found rather than as an
invalid chunk, so the part is retried later instead of being judged.
