...
Additionally, the value that is the closest to `max_receipt_size` is set to `max_receipt_size`:
...
params calculated for fours shards are (notice different base bandwidth):
