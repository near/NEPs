    ...
    FungibleTokenCore + FungibleTokenReceiver
{
    ...
    ///
    /// # Panics / Fails
    /// - The `convert_to_assets` method must never panic.
    ...
    ///
    /// # Panics / Fails
    /// - The `max_deposit` method must never panic.
    ...
    ///
    /// # Panics / Fails
    /// - The `preview_deposit` method must never panic.
    ...
    ///
    /// # Panics / Fails
    /// - The `max_mint` method must never panic.
    ...
    ///
    /// # Panics / Fails
    /// - The `preview_mint` method must never panic.
    ...
    ///
    /// # Panics / Fails
    /// - The `max_redeem` method must never panic.
    ...
    ///
    /// # Panics / Fails
    /// - The `max_withdraw` method must never panic.
    ...
    ///
    /// # Panics / Fails
    /// - The `preview_redeem` method must never panic.
    ...
    ///
    /// # Panics / Fails
    /// - The `preview_withdraw` method must never panic.
...
pub trait FungibleTokenReceiver {
    ...
    /// must invoke the `ft_on_transfer` callback.
    /// In the context of ERC-4626, both the **`deposit`** and **`mint`** functions
    /// are implemented through this entrypoint on the vault contract.
    ///
    /// While NEP-141 does not prescribe how the `msg` parameter should be structured,
    /// our suggested convention is to pass a JSON-encoded message with the following schema:
    ///
    /// ```rust
    ...
    /// ```
    ///
    /// Implementers are free to define their own schema for `msg`, but the vault must
...
- **Async Execution**:
  Because NEAR contracts execute asynchronously, results from `preview*` and `max*`
...
- **Optional Slippage Control**:
  To mitigate the risk of unfavorable execution due to block delays, vault contracts
  may optionally implement the following methods, which act as "slippage-protected"
  counterparts to `redeem` and `withdraw`.
...
pub trait VaultOptionalSlippageWithdraw {
    ...
    /// Withdraw a specific amount of assets, **only if** no more than
    /// `max_shares` are required to satisfy the request.
    /// This acts as a slippage-protected version of `withdraw`.
    fn withdraw_with_slippage(
        &mut self,
        assets: U128,
        receiver_id: Option<AccountId>,
        max_shares: U128
