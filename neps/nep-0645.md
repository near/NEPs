...
| Secret key: seed ξ (canonical) | 32 bytes |
| Secret key: expanded private key (legacy) | 4032 bytes |
...
### Secret-key string encoding

Secret keys never appear on chain, so this affects only key files, command-line tools and wallets. An ML-DSA-65 secret key is written as `ml-dsa-65:` followed by the base58 encoding of either:

- the 32-byte seed ξ that FIPS 204 `ML-DSA.KeyGen_internal` expands into the key pair (canonical), or
- the 4032-byte expanded private key that `ML-DSA.KeyGen_internal` outputs (legacy).

Decoders MUST tell the two apart by decoded length and MUST reject any other length. Implementations MUST accept the seed form and SHOULD generate, store and export keys in it ([RFC 9881] §8.1). An expanded key cannot be converted back to its seed, so implementations SHOULD also accept the expanded form. If they do, they MUST validate it on import and reject it on failure: every coefficient of s1 and s2 must be in [−η, η] (η = 4), and t0 and tr must match the values recomputed from ρ, s1 and s2 as in [FIPS 204] Algorithm 6. Recomputing only the public key is not enough, because a key with a corrupted tr or t0 still yields the right public key but can produce failing signatures. An imported expanded key MUST be exported unchanged.

| Example | Value |
| --- | --- |
| Seed ξ (hex) | `e82fe9d0fd2a6af0798736fc2ed44907faf66d4a7a543499e7f1c04f18a7d3c4` |
| Secret-key string | `ml-dsa-65:GdMxikMuRimoKStcDp78N8GXVK54z2HxgNhwKP6F3nMR` |
...
### 1.0.1 - Secret-key string encoding

- Specified the ML-DSA-65 secret-key string: the 32-byte seed (canonical) or the 4032-byte expanded key (legacy), told apart by length.
