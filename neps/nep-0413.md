...
To stop a malicious app from requesting the user to sign a message for them, only to relay it to a third-party. Including the recipient and making sure the user knows about it should mitigate this kind of attacks.
...
```jsx
sha256.hash(`NEP0413:` + `{"message":"hi","nonce":[0,...,31],"recipient":"myapp.com"}`)
```
