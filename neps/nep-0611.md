...
*Note*: So far, we ignored failed transactions in processing. After `ProtocolFeature::InvalidTxGenerateOutcomes`, they would impact the outcome but not the state. Therefore this NEP also introduces the change that failed transactions may update the state (to charge for gas).
