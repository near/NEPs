...
`DeleteAccount` can succeed even if the account has associated gas keys. Any gas key balances are burned along with the account.
...
* Cost of `TransferToGasKey` and `TransferFromGasKey` will be based on `Transfer`, as it is the most similar existing action. `TransferFromGasKey` and `TransferToGasKey` do additional trie modifications on sending and on execution. These will be accounted for based on trie operations.
