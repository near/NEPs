...
* `AddGasKey(PublicKey, AccessKeyPermission, u32)`: Creates a gas key with the given public key, 
...
`DeleteAccount` action will remove all associated gas keys. Any balance associated with a gas key during account deletion is burnt.

* For user's benefit we may consider failing delete account action if there are associated gas keys with a total balance exceeding a certain amount.

Note the reference implementation [already](https://github.com/near/nearcore/blob/136acc3a524575aae1300e26901e664adb521a6f/runtime/runtime/src/actions.rs#L662) prohibits deleting accounts over a certain storage threshold, which prevents a single action from doing an unbounded amount of work.
...
* Cost of `DeleteGasKey` will be based on the cost of deleting an access key, as it is the most similar existing action. In addition, the following fees will be charged per nonce to account for further trie operations:
  * `storage_remove_base`: Represent cost of adding a separate key to the trie,
  * `storage_remove_key_byte * len(Some(u32))`: Notably we only charge for the nonce portion of the key, as the trie nodes share a prefix with the gas key, and was already accounted for.
  * `storage_remove_ret_value_byte * len(Nonce)`
...
When a gas key is deleted, any remaining balance is burned rather than refunded. This prevents an attack where an adversary repeatedly creates and deletes gas keys to drain an account while only paying minimal transaction costs. 
...
* Existing use cases may assume they can submit an unbounded number of in-flight transactions (for example, they may rely on them being in the mempool). As changes only impact accounts with contracts, the impact of this is assumed to be low.
