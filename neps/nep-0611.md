...
Gas keys share a namespace with access keys, meaning that the user is not allowed to create a gas key with the same public key as an existing access key. This is important in refund handling.
...
`AddKey` action will fail if the public key is already registered as a gas key.
...
To facilitate the above design, gas keys must be stored in the trie.
As gas keys share a namespace with access keys, a natural way to store them is to store the gas keys under the same `TrieKey` as an access key:
...
Then, gas keys can be stored as a "special" kind of `AccessKeyPermission`:
...
While we can consider adding keys relatively uncommon, gas refunds are fairly common.
