...
1.  Alice queries a view-only method to determine if she already has storage on this contract with `ft::storage_balance_of({"account_id": "alice"})`. Using [NEAR CLI](https://docs.near.org/tools/cli) to make this view call, the command would be:
...
2.  Alice uses [NEAR CLI](https://docs.near.org/tools/cli) to make a view call.
