...
2. If the RPC returns a `MethodNotFound` error (the contract does not implement the method, or no contract is deployed), treat the `authorization` blob as a NEP-413 `SignedMessage` payload and verify it according to NEP-413, with the following purpose-binding rules:
   ...
   - Construct the **expected recipient string** as `"<PURPOSE>@<recipient>"`, where `<PURPOSE>` is the SCREAMING_SNAKE_CASE form of the requested purpose (e.g., `PROVE_OWNERSHIP`) and `<recipient>` is the recipient supplied to `w_resolve_auth` (e.g., `example.app`). For example: `"PROVE_OWNERSHIP@example.app"`.
...
Alice's NEAR account is `0s1234...` — a deterministic account ID on which an EIP-712 wallet contract is deployed. The contract is bound to her Ethereum address `0x1234...`, controlled by MetaMask. There are no NEAR access keys on `0s1234...`; the wallet contract is the sole authority. She wants to log into `example.app`.
...
The `account_id` is the NEAR wallet-contract account `0s1234...`; the ECDSA signature is produced by the controlling Ethereum address `0x1234...` (MetaMask). The wallet contract on `0s1234...` knows its configured controlling address is `0x1234...` and verifies the recovered signer matches.
...
sequenceDiagram
    ...
    par member1.near (regular NEAR account)
        ...
        N413->>M1User: sign NEP-413 with recipient =<br/>"APPROVE_OFFCHAIN_ACTION@<br/>orderbook.example.com"
    ...
    and sub-multisig.near (nested multisig)
        ...
        N413->>S1User: sign NEP-413 with recipient =<br/>"APPROVE_OFFCHAIN_ACTION@<br/>orderbook.example.com"
...
sequenceDiagram
    ...
    RPC->>DAO: w_resolve_auth(APPROVE_OFFCHAIN_ACTION,<br/>"orderbook.example.com", root_blob)
    ...
    RPC->>M1: w_resolve_auth(APPROVE_OFFCHAIN_ACTION,<br/>"orderbook.example.com", blob_1)
    ...
    RPC->>PK: w_resolve_auth(APPROVE_OFFCHAIN_ACTION,<br/>"orderbook.example.com", blob_2)
    ...
    RPC->>SUB: w_resolve_auth(APPROVE_OFFCHAIN_ACTION,<br/>"orderbook.example.com", blob_3)
    ...
    RPC->>S1: w_resolve_auth(APPROVE_OFFCHAIN_ACTION,<br/>"orderbook.example.com", blob_3_1)
