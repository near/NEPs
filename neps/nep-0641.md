...
pub struct OffchainMessage {
    ...
    /// (absent from JSON, still a four-byte zero count in Borsh).
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub path: Vec<AccountId>,

    /// UNIX timestamp at signing
    /// (RFC-3339 string in JSON, `u64` nanoseconds in Borsh).
    /// No fractional part when nanoseconds are zero, else nine digits
    /// with trailing zeros stripped. Pre-epoch instants fail to serialize.
...
4. The signature verifies per `via` against `access_key`:
   - **ed25519** → reject small-order public keys, then verify with the **cofactorless** equation, comparing compressed `R` bytes. Small-order rejection is layered above protocol-level verification, which accepts such keys; see the [reference implementation](#reference-implementation).
   - **secp256k1** → the recovery byte is range-checked to 0 to 3; any other value is rejected at parse time.
