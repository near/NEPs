...
Authors: Vlad Frolov, Arseny Mitin <mitinarseny@gmail.com>
Status: Final
...
In the RFC-3339 form, no fractional part is emitted when the nanosecond component is zero; otherwise nanoseconds are padded to nine digits and trailing zeros are stripped, keeping at least one digit (`2026-08-05T07:28:00Z`, `2026-08-05T07:28:00.1Z`, `2026-08-05T07:28:00.123456789Z`). The Borsh form is `u64` nanoseconds, so instants before the UNIX epoch cannot be represented and fail to serialize.
...
The `skip_serializing_if` on `path` governs JSON only: an empty `path` is absent from the JSON form but still contributes its four-byte little-endian count of zero to `borsh(msg)`, and therefore to the hash.
...
4. The signature verifies per `via` against `access_key`:
   - **ed25519** → reject small-order public keys, then verify with the **cofactorless** equation, comparing compressed `R` bytes; implementations must match this, since the cofactored equation most high-level libraries expose by default accepts signatures the reference rejects.
   - **secp256k1** → the recovery byte is range-checked and rejected unless it is 0 to 3, even though verification never recovers a public key.
...
Sibling pending entries MAY resolve concurrently. Finally, the dApp MUST check the returned payload by either checking that it equals the one it issued or validating the payload (e.g. domain to match the dApp, action against user permissions, and nonce material preventing replays).
