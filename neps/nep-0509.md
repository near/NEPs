...
Let's formalise a proposed change to the validator roles and responsibilities, with same and new behavior clearly labelled:
...
`sample_chunk_validators` basically just shuffles `validator_mandates` among shards using height-specific seed.  
...
The minor accuracy needed is that state transition proofs needs to be saved one epoch in advance, so we won't have to re-apply chunks to generate proofs once stateless validation is enabled. But new epoch protocol version is defined by finalization of **previous previous epoch**, so this is fine.
