...
The plan for new storage read costs is to essentially drop TTN and cover it with `storage_read_base`, which should
...
contracts. So we are likely to agree on 100 Ggas and plan to implement inlining separately. Note that cost reduction, if
it happens, won't be significant, because in fact 16 Ggas per TTN is significantly off.
