...
The BLS12-381[^1],[^11],[^52] is a widely used[^2],[^3],[^4],[^5],[^6],[^7] elliptic curve with 120+ bits security level[^8], which supports *the pairing operation*. It is an alternative for the BN254 elliptic curve[^9],[^12], which also supports the aggregation and has already been implemented on NEAR as host functions[^10]. Recent research shows that the BN254 security level is lower than 100-bit[^13]. Also, there is a tendency to switch from BN254 to BLS12-381 in the industry – ZCash[^14], Ethereum[^15],[^3], Tezos[^16],[^5] or just use it – Filecoin[^6].
